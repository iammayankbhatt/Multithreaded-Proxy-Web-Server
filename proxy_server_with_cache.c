...
typedef struct cache_element{
    ...
    struct cache_element* next;  // for making linkedlist of cache elements
}cache_element;
...
void* thread_function(void* socketNew){
   ...
   sem_getvalue(&semaphore, &p);
    printf("Number of clients in the proxy server: %d\n", MAX_CLIENTS - p);
    printf("Semaphore value : %d\n",p);
    int*t = (int*)socketNew;
    int socket = *t;
    int bytes_send_client, len;
    char* buffer = (char*)calloc(MAX_BYTES, sizeof(char)); 
    bzero(buffer, MAX_BYTES); // to clear the buffer or garbage removal
    ...
    if(temp!=NULL){
        ...
        if(ParsedRequest_parse(request, buffer, len)<0){
            ...
            if(!strcmp(request->method,"GET")){
                ...
                printf("Thos code doesn't support any other method apart from GET\n");
            }
        }
...
int main(int argc, char* argv[]){
    ...
    proxy_socket_id = socket(AF_INET, SOCK_STREAM,0);   // sockstream for TCP , af_inet for ipv4
    ...
    if(bind(proxy_socket_id, (struct sockaddr*)&server_addr, sizeof(server_addr))<0){
        perror("Bind failed, port not available. \n");
        exit(1);
    ...
    while(1){
        ...
        inet_ntop( AF_INET, &ip_addr, str, INET_ADDRSTRLEN); // it converts the network address structure to string
        printf("Client connected from IP address: %s with port number :%d\n", str,ntohs(client_addr.sin_port));
        pthread_create(&tid[i],NULL, thread_function, (void*)&Connected_socketId[i]);
        i++;
